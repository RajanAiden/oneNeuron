...
import joblib  # fo saving my model as a binary file
...
def prepare_data(df):
  """It is used to spearate the dependent variable and independent features 
  ...
  """
  logging.info("preparing the data by segreating independet and dependent variables")
  x = df.drop("y", axis=1)
  y = df["y"]
  return x,y

def save_model(model, filename): # for saving your trained model
  """it saves the trained model

  Args:
      model (python object): object of the trained model
      filename (str): path to save the trained model
  """
  logging.info("saving the trained model")
  model_dir = 'models'
  os.makedirs(model_dir, exist_ok=True) # only ceate if model_dir doesn't exist
...
def save_plot(df, file_name, model):
  """
  :param df: its the pandas dataframe
